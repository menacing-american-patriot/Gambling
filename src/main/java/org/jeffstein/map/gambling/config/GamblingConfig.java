package org.jeffstein.map.gambling.config;
...
public class GamblingConfig {
    ...
    private void validateConfig() {
        ...
        // Ensure per-game enabled flags exist (default: true)
        String[] gameKeys = {
            "blackjack","roulette","slots","poker","baccarat",
            "craps","keno","crash","plinko","wheel","mines","coinflip"
        };
        for (String key : gameKeys) {
            String path = "games." + key + ".enabled";
    ...
    public boolean isGameEnabled(String key) {
        String base = "games." + key.toLowerCase() + ".enabled";
        if (config.contains(base)) {
            return config.getBoolean(base, true);
        }
        // Fallback aliases
        if (key.equalsIgnoreCase("wheel")) {
            return config.getBoolean("games.wheel_of_fortune.enabled", true);
        }
        return true;
    }

    public boolean isGameEnabled(CasinoGameType type) {
        String key;
        switch (type) {
            case WHEEL_OF_FORTUNE: key = "wheel"; break;
            default: key = type.name().toLowerCase();
        }
        return isGameEnabled(key);
    }
